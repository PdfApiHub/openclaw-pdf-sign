...
function getApiKey(config: Record<string, unknown>): string {
  ...
  if (!key) {
    throw new Error(
      "PDFAPIHub API key not configured. Add your key in plugin config (plugins.entries.pdf-sign.env.PDFAPIHUB_API_KEY) or set apiKey in plugin config. Get a free key at https://pdfapihub.com"
    );
  }
  return key;
}
